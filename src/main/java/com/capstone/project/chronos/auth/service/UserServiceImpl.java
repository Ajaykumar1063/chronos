...
import com.capstone.project.chronos.auth.repository.VerificationTokenRepository;
...
public class UserServiceImpl implements UserService {
    ...
    public User registerUser(UserModel userModel, HttpServletRequest request) throws TokenExpiredException {
        try {
            User existingUser = _userRepository.findByEmail(userModel.getEmail()).orElse(null);
            User user;
            if (existingUser != null) {
                throw new TokenExpiredException("User already exists with this email");
            } else {
                user = User.builder()
                        .password(passwordEncoder.encode(userModel.getPassword()))
                        .email(userModel.getEmail()).firstName(userModel.getFirstName())
                        .lastName(userModel.getLastName()).isEnabled(false).createdAt(LocalDate.now()).build();
                if (user != null) {
